...
平台数据全部来自各平台**开放接口**。没有配置开发者凭据时，系统明确显示「未接入 / 暂无可核验数据」，**绝不虚构价格、优惠、政策或评测观点**。
...
app/
  ...
  routers/       API 路由：search / reviews / sources / health
  database.py    SQLAlchemy 2.0 异步 ORM（默认 SQLite）
frontend/
  src/pages/     首页、搜索、商品详情、横向对比、评测库、数据来源
  src/components/ 设计系统与业务组件（表格/卡片双形态、拆解、推荐面板、评测卡片）
  src/lib/       API 客户端、类型、格式化与中文标签映射
tests/           56 个用例：价格、叠加、匹配、推荐、API、HTTP 客户端
```

分层方向：`domain`（纯逻辑，可单测）→ `adapters`（外部数据）→ `services`（编排）→ `routers`（HTTP）→ 前端。
...
测试覆盖重点：到手价三层的边界（含运费、互斥、以旧换新、演示折扣）、规格硬冲突不合并、推荐打分与「数据不足时拒答」、搜索/详情/对比 API 的诚实降级、HTTP 客户端对非 http(s) 与内网地址的拦截。

---

## 8. 诚实的边界（系统明确不做的事）

- **不抓取需登录的商品页面**：个性化价格、登录后优惠券、下单流程中的最终优惠需要账号环境；系统只用开放接口与公开规则，不绕过登录、验证码与反爬机制。
...
- **未接入平台不虚构**：缺少凭据时返回「未接入」与申请指引，而不是假数据。
...
**后端** FastAPI · Pydantic v2 · SQLAlchemy 2.0（异步）· SQLite · httpx · loguru · cachetools
