    ...
    <div className="stack gap-24">
      <header className="stack gap-12">
        ...
        <div className="row gap-16 wrap small muted">
          ...
          <span>
            <Icon name="review" size={13} /> 模型调用 {task.model_usage.calls} 次
            {task.model_usage.vision_calls > 0 && `（其中视觉 ${task.model_usage.vision_calls} 次）`}
          </span>
          <span>
            估算费用 ¥{task.model_usage.estimated_cost.toFixed(4)}（仅为估算，不等于最终账单）
          </span>
          {task.model_usage.errors.length > 0 && (
            <span className="text-price">模型调用失败 {task.model_usage.errors.length} 次</span>
          )}
        </div>
      </header>
