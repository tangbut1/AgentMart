...
def _assert_usage_complete(usage: dict, where: str) -> None:
    """model_usage 必须给全字段。

    前端按 usage.estimated_cost.toFixed(4) 渲染，缺一个字段
    整个任务页就会白屏（用户看到的是一片空白，没有任何提示）。
    """
    missing = [f for f in MODEL_USAGE_FIELDS if f not in usage]
    assert not missing, f"{where} 的 model_usage 缺字段：{missing}"
    assert isinstance(usage["errors"], list)


@pytest.mark.asyncio
async def test_new_task_has_complete_model_usage():
    agent = _agent()
    task = _task(agent)
    _assert_usage_complete(task.to_dict()["model_usage"], "刚创建的任务")
    await agent.cancel_task(task.id)


@pytest.mark.asyncio
async def test_task_with_no_offers_still_reports_complete_usage(db):
    """没取到任何真实商品时最容易漏：_finalize 提前返回。"""
    agent = _agent()
    task = _task(agent)
    await agent.start(task.id)
    agent.answer_question(task.id, "include_reviews", "不要")
    ...
    for _ in range(120):
        ...
        await asyncio.sleep(0.05)
    assert task.status.is_terminal
    assert task.real_offers == [], "脚本驱动下取不到商品，正是要复现的场景"

    _assert_usage_complete(task.to_dict()["model_usage"], "没有商品的任务")
    assert task.model_usage["calls"] == 0

    async with db() as session:
        await store.save_task(session, task)
        record = await store.get_task_record(session, task.id)
        _assert_usage_complete(record["model_usage"], "落库记录")
        _assert_usage_complete(
            store.task_view_from_record(record)["model_usage"], "重启回读视图"
        )


@pytest.mark.asyncio
async def test_record_with_legacy_empty_usage_is_backfilled(db):
    """老库里存的是 {}，回读时要补成完整形状。"""
    agent = _agent()
    task = _task(agent)
    async with db() as session:
        await store.save_task(session, task)
        # 手动改成老数据：model_usage 是空 JSON 对象
        from sqlalchemy import update

        from app.browser.store import BrowserTaskRecord

        await session.execute(
            update(BrowserTaskRecord)
            .where(BrowserTaskRecord.id == task.id)
            .values(model_usage="{}")
        )
        await session.commit()

        record = await store.get_task_record(session, task.id)
        _assert_usage_complete(record["model_usage"], "老记录回读")
        _assert_usage_complete(
            store.task_view_from_record(record)["model_usage"], "老记录视图"
        )
